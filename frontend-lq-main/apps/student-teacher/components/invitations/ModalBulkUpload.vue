<template>
  ...
  >
    <div class="flex flex-col gap-6">
      ...
      <div class="bg-primary-50 rounded-2xl p-6 border-2 border-primary-200">
        <div class="flex items-start gap-4 mb-6">
          <div class="w-12 h-12 bg-primary-500 rounded-xl flex items-center justify-center shrink-0">
            <Icon name="solar:document-line-duotone" class="text-white text-xl" />
          </div>
      ...
      <!-- Error Message -->
      <Message v-if="errorMessage" :severity="hasPartialSuccess ? 'warn' : 'error'" :closable="false" class="w-full">
        <div class="flex flex-col gap-2">
          <div class="font-semibold">
            {{
              hasPartialSuccess
                ? $t("invitations.bulkUpload.errorPartialTitle")
                : $t("invitations.bulkUpload.errorTitle")
            }}
          </div>
          <div class="text-sm whitespace-pre-line max-h-60 overflow-y-auto">
            {{ errorMessage }}
          </div>
          <div class="text-xs text-surface-600 mt-2">
            {{ $t("invitations.bulkUpload.errorInstructions") }}
          </div>
        </div>
      </Message>
      ...
      <div
        :class="[
          ...
          errorMessage && !hasPartialSuccess ? 'border-danger-300 bg-danger-50' : '',
        ]"
      ...
      >
        <div
          :class="[
            ...
            errorMessage && !hasPartialSuccess ? 'bg-danger-100' : 'bg-primary-100',
          ]"
        >
          <Icon
            :name="errorMessage && !hasPartialSuccess ? 'solar:close-circle-line-duotone' : 'solar:upload-line-duotone'"
            :class="['text-3xl', errorMessage && !hasPartialSuccess ? 'text-danger-600' : 'text-primary-600']"
          />
        </div>
...
<script setup lang="ts">
  ...
  // Clear error when a new file is selected
  watch(
    () => selectedFile.value,
    (newFile) => {
      if (newFile) {
  ...
  function handleDrop(event: DragEvent): void {
    ...
    if (file && isValidFileType(file)) {
      selectedFile.value = file;
      // Clear error when new file is dropped
      emit("clear-error");
  ...
  function onSubmit(): void {
    if (selectedFile.value && !props.loading) {
      ...
      // Don't clear the file immediately - let the user see what they uploaded
      // Only clear if the upload is successful (handled by parent)
      // Reset input to allow selecting the same file again if there's an error
      if (fileInputRef.value) {
        fileInputRef.value.value = "";
      }
    }
